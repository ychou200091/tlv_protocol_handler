...
本專案目標是使用 C 實作一個穩健的解析器，能夠將二進制流 (Byte Stream) 轉換為結構化的資料。
...
* Type: 0x02 (ID), Length: 0x02, Value: 0xD431

**完整原始數據 (Hex):** `01 0B 54 61 69 77 61 6E 39 38 37 36 35 02 02 D4 31`
...
* 給定一個特定的 Type，解析器需快速回傳對應的 Value 指標。

### C. 序列化功能 (Serialization)
...
* **平台無關性:** 不使用特定作業系統的 Library，確保程式碼能在 Bare-metal 嵌入式環境執行。
